export namespace MCProtocol.Bedrock_1_26_40 {
	...
	export type maybeIncompleteArray = unique symbol;
	export type optionalOnRemaining = unique symbol;
	export type nbt = any;
	...
	export type MetadataFlags2 = {
		...
		uses_legacy_friction?: boolean;
		uses_uniform_air_drag?: boolean;
		nameplate_depth_tested?: boolean;
		not_pickable_from_inside?: boolean;
	};
	export type Link = {
	...
	export type TransactionActions = {
		...
		container_presence: boolean;
		window_id?: number;
		flag_presence: boolean;
		flags?: number;
		slot: number;
		old_item: ItemV4;
		new_item: ItemV4;
	...
	export type SmithingTransformRecipe = {
		/** RecipeID is a unique ID of the recipe. This ID must be unique amongst all other types of recipes too, but its functionality is not exactly known. */
		recipe_id: LatinString;
		...
		tag: string;
		/** RecipeNetworkID is a unique ID used to identify the recipe over network. Each recipe must have a unique network ID. Recommended is to just increment a variable for each unique recipe registered. This field must never be 0. */
		network_id: number;
	};
	 ...
	 */
	export type PersonaPieceType = "unknown" | "skeleton" | "body" | "skin" | "bottom" | "feet" | "dress" | "top" | "high_pants" | "hands" | "outerwear" | "facial_hair" | "mouth" | "eyes" | "hair" | "hood" | "back" | "face_accessory" | "head" | "legs" | "left_leg" | "right_leg" | "arms" | "left_arm" | "right_arm" | "capes" | "classic_skin" | "emote" | "unsupported";
	export type Skin = {
		skin_id: string;
		play_fab_id: string;
	 ...
	 */
	export type PlayerRecords = `$$maybeIncompleteArray`;
	export type Enchant = {
		id: number;
		level: number;
	};
	...
	export type StackRequestSlotInfo = {
		...
		slot: number;
		/** StackNetworkID is the unique stack ID that the client assumes to be present in this slot. The server must check if these IDs match. If they do not match, servers should reject the stack request that the action holding this info was in. As of 1.26.40 this is a little endian int instead of a zigzag varint. */
		stack_id: number;
	};
	/**
	 ...
	 * little endian short.
	 */
	export type RecipeIngredient2 = {
		type: "invalid" | "name" | "molang" | "item_tag";
	 ...
	 */
	export type ItemStackRequestInstanceDescriptor = {
		type: "invalid" | "name";
	...
	export type ItemStackRequest = {
		...
		request_id: number;
		actions: {
			/** As of 1.26.40 the action type is an unsigned varint, the 'place in container' and 'take out container' entries are gone, and the type is repeated as a byte right after it. */
			type_id: "take" | "place" | "swap" | "drop" | "destroy" | "consume" | "create" | "lab_table_combine" | "beacon_payment" | "mine_block" | "craft_recipe" | "craft_recipe_auto" | "craft_creative" | "optional" | "craft_grindstone_request" | "craft_loom_request" | "non_implemented" | "results_deprecated";
			...
			ingredients?: RecipeIngredient2[];
			item_id?: number;
			filtered_string_index?: number;
			cost?: number;
			pattern?: string;
			result_items?: ItemStackRequestInstanceDescriptor[];
		}[];
		custom_names: string[];
	...
	export type ItemStackResponses = {
		...
		request_id: number;
		/** As of 1.26.40 the container list no longer depends on the status: it is preceded by a presence byte (always written as true) and is then written as an optional. */
		containers_presence: boolean;
		containers?: {
			slot_type: FullContainerName;
			slots: {
				slot: number;
				hotbar_slot: number;
				count: number;
				item_stack_id_presence: boolean;
	...
	export type MapDecoration = {
		...
		label: string;
		/** Colour is the colour of the map decoration. Some map decoration types have a specific colour set automatically, whereas others may be changed. As of 1.26.40 this is a little endian int. */
		color_abgr: number;
	};
	...
	export type WindowIDZigzag32 = "inventory" | "first" | "last" | "offhand" | "armor" | "creative" | "hotbar" | "fixed_inventory" | "ui" | "drop_contents" | "beacon" | "trading_output" | "trading_use_inputs" | "trading_input_2" | "trading_input_1" | "enchant_output" | "enchant_material" | "enchant_input" | "anvil_output" | "anvil_result" | "anvil_material" | "container_input" | "crafting_use_ingredient" | "crafting_result" | "crafting_remove_ingredient" | "crafting_add_ingredient" | "none";
	...
	export type CameraSplineDefinition = {
		name: string;
		total_time: number;
		spline_type?: string;
		control_points: vec3f[];
		progress_key_frames: CameraProgressOption[];
		rotation_key_frames: CameraRotationOption[];
	};
	export type CameraAimAssistActorPriorityData = {
		preset_index: number;
		category_index: number;
		actor_index: number;
		priority: number;
	};
	export type MemoryCategoryCounter = {
		category: "unknown" | "invalid_size_unknown" | "actor" | "actor_animation" | "actor_rendering" | "block_ticking_queues" | "biome_storage" | "cereal" | "circuit_system" | "client" | "commands" | "db_storage" | "debug" | "documentation" | "ecs_systems" | "fmod" | "fonts" | "im_gui" | "input" | "json_ui" | "json_ui_control_factory_json" | "json_ui_control_tree" | "json_ui_control_tree_control_element" | "json_ui_control_tree_populate_data_binding" | "json_ui_control_tree_populate_focus" | "json_ui_control_tree_populate_layout" | "json_ui_control_tree_populate_other" | "json_ui_control_tree_populate_sprite" | "json_ui_control_tree_populate_text" | "json_ui_control_tree_populate_tts" | "json_ui_control_tree_visibility" | "json_ui_create_ui" | "json_ui_defs" | "json_ui_layout_manager" | "json_ui_layout_manager_remove_dependencies" | "json_ui_layout_manager_init_variable" | "languages" | "level" | "level_structures" | "level_chunk" | "level_chunk_gen" | "level_chunk_gen_thread_local" | "light_volume_manager" | "network" | "marketplace" | "material_dragon_compiled_definition" | "material_dragon_material" | "material_dragon_resource" | "material_dragon_uniform_map" | "material_render_material" | "material_render_material_group" | "material_variation_manager" | "molang" | "ore_ui" | "persona" | "player" | "render_chunk" | "render_chunk_index_buffer" | "render_chunk_vertex_buffer" | "rendering" | "rendering_library" | "request_log" | "resource_packs" | "sound" | "sub_chunk_biome_data" | "sub_chunk_block_data" | "sub_chunk_light_data" | "textures" | "vr" | "weather_renderer" | "world_generator" | "tasks" | "test" | "scripting" | "scripting_runtime" | "scripting_context" | "scripting_context_bindings_mc" | "scripting_context_bindings_gt" | "scripting_context_run" | "data_driven_ui" | "data_driven_ui_defs" | "gameface" | "gameface_system" | "gameface_dom" | "gameface_css" | "gameface_display" | "gameface_temp_allocator" | "gameface_pool_allocator" | "gameface_dump" | "gameface_media" | "gameface_json" | "gameface_script_engine";
		bytes: bigint;
	};
	export type VoxelCells = {
		x_size: number;
		y_size: number;
		z_size: number;
		storage: number[];
	...
	export type VoxelShapeNameEntry = {
		name: string;
		id: number;
	};
	export type GatheringJoinInfo = {
	...
	export type StoreEntryPointInfo = {
		store_id: string;
		store_name: string;
	};
	 ...
	 */
	export type BiomeSurfaceBuilder = {
	 ...
	 */
	export type BiomeNoiseGradientSurface = {
		non_replaceable_blocks: number[];
		gradient_blocks: BiomeNoiseBlockSpecifier[];
		noise_seed: string;
		first_octave: number;
		amplitudes: number[];
	};
	 ...
	 */
	export type BiomeNoiseBlockSpecifier = {
		noise: string;
		threshold: number;
		min: number;
		max: number;
	...
	export type EaseType = "Linear" | "Spring" | "InQuad" | "OutQuad" | "InOutQuad" | "InCubic" | "OutCubic" | "InOutCubic" | "InQuart" | "OutQuart" | "InOutQuart" | "InQuint" | "OutQuint" | "InOutQuint" | "InSine" | "OutSine" | "InOutSine" | "InExpo" | "OutExpo" | "InOutExpo" | "InCirc" | "OutCirc" | "InOutCirc" | "InBounce" | "OutBounce" | "InOutBounce" | "InBack" | "OutBack" | "InOutBack" | "InElastic" | "OutElastic" | "InOutElastic" | "InverseLerp";
	...
	export type GraphicsOverrideParameterType = "sky_zenith_color" | "sky_horizon_color" | "horizon_blend_min" | "horizon_blend_max" | "horizon_blend_start" | "horizon_blend_mie_start" | "rayleigh_strength" | "sun_mie_strength" | "moon_mie_strength" | "sun_glare_shape" | "chlorophyll" | "cdom" | "suspended_sediment" | "waves_depth" | "waves_frequency" | "waves_frequency_scaling" | "waves_speed" | "waves_speed_scaling" | "waves_shape" | "waves_octaves" | "waves_mix" | "waves_pull" | "waves_direction_increment" | "midtones_contrast" | "highlights_contrast" | "shadows_contrast" | "highlights_gain" | "highlights_gamma" | "highlights_offset" | "highlights_saturation" | "midtones_gain" | "midtones_gamma" | "midtones_offset" | "midtones_saturation" | "shadows_gain" | "shadows_gamma" | "shadows_offset" | "shadows_saturation" | "highlights_min" | "shadows_max" | "temperature" | "sun_color" | "sun_illuminance" | "moon_color" | "moon_illuminance" | "flash_color" | "flash_illuminance" | "ambient_color" | "ambient_illuminance" | "emissive_desaturation" | "sky_intensity" | "orbital_offset_degrees";
	...
	export type DataStorePropertyValue = {
		type: "none" | "bool" | "int64" | "string" | "map";
		value: boolean | bigint | string | DataStoreMapEntry[];
	};
	export type DataStoreMapEntry = {
		key: string;
		value: DataStorePropertyValue;
	};
	export type DataStoreChangeEntry = {
		change_type: "update" | "change" | "removal";
		name?: string;
		property?: string;
		path?: string;
	...
	export type WaypointWorldPosition = {
		position: vec3f;
		dimension_id: number;
	};
	export type Waypoint = {
		...
		icon_size?: vec2f;
		color?: number;
		client_position_authority?: boolean;
		actor_unique_id?: bigint;
	};
	export type LocatorBarWaypoint = {
		group_handle: string;
		waypoint: Waypoint;
		action: "none" | "add" | "remove" | "update";
	};
	export type SyncWorldClockStateData = {
		clock_id: bigint;
		time: number;
		paused: boolean;
	};
	export type TimeMarkerData = {
		id: bigint;
		name: string;
		time: number;
		period?: number;
	...
	export type AttributeData = {
		type: "bool" | "float" | "colour";
		bool_value?: boolean;
		bool_operation?: number | undefined;
		float_value?: number;
		float_operation?: number | undefined;
		float_constraint_min?: number | undefined;
		float_constraint_max?: number | undefined;
		colour_value?: number;
		colour_operation?: number | undefined;
	};
	export type EnvironmentAttributeData = {
		attribute_name: string;
		from_attribute?: AttributeData;
		attribute: AttributeData;
		to_attribute?: AttributeData;
		current_transition_ticks: number;
		total_transition_ticks: number;
		ease_type: number;
		local_transition_ticks: number;
		noise_transition: boolean;
	};
	export type AttributeLayerData = {
		name: string;
		noise_name?: string;
		dimension_id: number;
	...
	export type EntityDiagnosticTimingInfo = {
		display_name: string;
		entity: string;
		duration_nanos: bigint;
		percent_of_total: number;
	};
	export type SystemDiagnosticTimingInfo = {
		display_name: string;
		system_index: bigint;
		duration_nanos: bigint;
		percent_of_total: number;
	...
	export type WhiskerScopeDataSummary = {
		label: string;
		indentation: string;
		total_high_cost_ns: bigint;
		total_mid_cost_ns: bigint;
		total_low_cost_ns: bigint;
	};
	export type PrimitiveShape = {
		network_id: bigint;
		type?: "line" | "box" | "sphere" | "circle" | "text" | "arrow";
		location?: vec3f;
		scale?: number;
		rotation?: vec3f;
		total_time_left?: number;
		max_render_distance?: number;
		color?: number;
		dimension_id?: number;
		attached_to_entity_id?: bigint;
		extra_shape_data: ShapeLine | ShapeBox | ShapeSphere | ShapeCircle | ShapeText | ShapeArrow;
	};
	...
	export type ShapeText = {
		...
		depth_test?: boolean;
		show_backface?: boolean;
		show_backface_text?: boolean;
	};
	...
	export type packet_resource_pack_client_response = {
		/** As of 1.26.40 the status is an unsigned varint and no longer includes the 'none' entry. It is followed by the same status as a string, and the pack IDs are only written for 'send_packs'. */
		response_status: "refused" | "send_packs" | "have_all_packs" | "completed";
	...
	export type packet_start_game = {
		...
		/** Some Minecraft: Education Edition field that specifies what 'region' the world was from, with 0 being None, 1 being RestOfWorld, and 2 being China. The actual use of this field is unknown. As of 1.26.40 this is an unsigned varint. */
		edu_offer: number;
		...
		/** Defines game rules currently active with their respective values. The value of these game rules may be either 'bool', 'Int32' or 'Float32'. Some game rules are server side only, and don't necessarily need to be sent to the client. As of 1.26.40 StartGame uses the same game rule encoding as the rest of the protocol (int values are little endian ints instead of varints). */
		gamerules: GameRuleI32[];
		experiments: Experiments;
		experiments_previously_used: boolean;
		...
		server_join_info?: ServerJoinInformation;
	...
	export type packet_crafting_data = {
		/** As of 1.26.40 recipes are no longer sent as one tagged array; each recipe kind has its own array and the recipe type is implied by which array the recipe appears in. */
		shaped_recipes: ShapedRecipe[];
		shapeless_recipes: ShapelessRecipe[];
		multi_recipes: MultiRecipe[];
		/** Shulker box recipes, which are shapeless. */
		shulker_box_recipes: ShapelessRecipe[];
		shapeless_chemistry_recipes: ShapelessRecipe[];
		shaped_chemistry_recipes: ShapedRecipe[];
		smithing_transform_recipes: SmithingTransformRecipe[];
		smithing_trim_recipes: SmithingTrimRecipe[];
	...
	export type packet_boss_event = {
		...
		title: string;
		/** FilteredBossBarTitle is a filtered version of BossBarTitle with all the profanity removed. The client will use this over BossBarTitle if this field is not empty and they have the "Filter Profanity" setting enabled. */
		...
		color: "pink" | "blue" | "red" | "green" | "yellow" | "purple" | "rebecca_purple" | "white";
	...
	export type packet_structure_block_update = {
		...
		/** RedstoneSaveMode is the mode that should be used to save the structure when used with redstone. In Java Edition, this is always stored in memory, but in Bedrock Edition it can be stored either to disk or memory. See the constants above for the options. As of 1.26.40 this is a single byte instead of a zigzag varint. */
	...
	export type packet_set_score = {
		entries: {
			entry_type: "remove" | "player" | "entity" | "fake_player";
			entry_type_name: string;
			scoreboard_id: bigint;
			objective_name?: string | undefined | string;
			score?: number;
			custom_name?: string;
			entity_unique_id?: bigint;
		}[];
	...
	export type packet_player_auth_input = {
		...
		delta: vec3f;
		/** As of 1.26.40 none of the fields below are gated on the input flags any more. Each is preceded by a presence byte (always written as true) and is then written as its own optional. */
		transaction_presence: boolean;
		transaction?: {
			legacy: TransactionLegacy;
			actions_presence: boolean;
			actions?: TransactionActions;
	 ...
	 */
	export type InputData = "ascend" | "descend" | "north_jump" | "jump_down" | "sprint_down" | "change_height" | "jumping" | "auto_jumping_in_water" | "sneaking" | "sneak_down" | "up" | "down" | "left" | "right" | "up_left" | "up_right" | "want_up" | "want_down" | "want_down_slow" | "want_up_slow" | "sprinting" | "ascend_block" | "descend_block" | "sneak_toggle_down" | "persist_sneak" | "start_sprinting" | "stop_sprinting" | "start_sneaking" | "stop_sneaking" | "start_swimming" | "stop_swimming" | "start_jumping" | "start_gliding" | "stop_gliding" | "item_interact" | "block_action" | "item_stack_request" | "handled_teleport" | "emoting" | "missed_swing" | "start_crawling" | "stop_crawling" | "start_flying" | "stop_flying" | "received_server_data" | "client_predicted_vehicle" | "paddling_left" | "paddling_right" | "block_breaking_delay_enabled" | "horizontal_collision" | "vertical_collision" | "down_left" | "down_right" | "start_using_item" | "camera_relative_movement_enabled" | "rot_controlled_by_move_direction" | "start_spin_attack" | "stop_spin_attack" | "hotbar_only_touch" | "jump_released_raw" | "jump_pressed_raw" | "jump_current_raw" | "sneak_released_raw" | "sneak_pressed_raw" | "sneak_current_raw" | "internal_update";
	/**
	...
	export type packet_creative_content = {
		...
		groups: {
			/** Where the group shows up in the creative inventory (e.g. Construction, Items, etc) As of 1.26.40 this is a single byte instead of a little endian int. */
	...
	export type packet_set_player_inventory_options = {
		...
		filtering: boolean;
		layout: "none" | "inventory_only" | "default" | "recipe_book_only";
		crafting_layout: "none" | "inventory_only" | "default" | "recipe_book_only";
	};
	/**
	...
	export type packet_clientbound_data_driven_ui_show_screen = {
		screen_id: string;
		form_id: number;
		data_instance_id?: number;
	};
	export type packet_clientbound_data_driven_ui_close_screen = {
		form_id?: number;
	};
	export type packet_clientbound_data_driven_ui_reload = {
		
	};
	export type packet_clientbound_texture_shift = {
		action: "invalid" | "initialize" | "start" | "set_enabled" | "sync";
		collection_name: string;
		from_step: string;
		to_step: string;
		all_steps: string[];
		current_length_ticks: bigint;
		total_length_ticks: bigint;
		enabled: boolean;
	};
	export type packet_voxel_shapes = {
		shapes: VoxelShape[];
		name_map: VoxelShapeNameEntry[];
		custom_shape_count: number;
	};
	export type packet_camera_spline = {
		splines: CameraSplineDefinition[];
	};
	export type packet_camera_aim_assist_actor_priority = {
		priority_data: CameraAimAssistActorPriorityData[];
	};
	export type packet_resource_packs_ready_for_validation = {
		
	};
	export type packet_locator_bar = {
		waypoints: LocatorBarWaypoint[];
	};
	export type packet_party_changed = {
	...
	export type packet_sync_world_clocks = {
		payload_type: "sync_state" | "initialize_registry" | "add_time_marker" | "remove_time_marker";
		sync_states?: SyncWorldClockStateData[];
		clocks?: WorldClockData[];
		add_clock_id?: bigint;
		add_time_markers?: TimeMarkerData[];
		remove_clock_id?: bigint;
		remove_time_marker_ids?: bigint[];
	};
	export type packet_clientbound_attribute_layer_sync = {
		payload_type: "update_layers" | "update_settings" | "update_environment" | "remove_environment";
	...
	export type packet_server_store_info = {
	...
	export type packet_party_destination_cookie_response = {
		cookie: string;
		accepted: boolean;
	};
	...
	export interface ClientboundPacketEventMap {
		...
		camera_aim_assist_actor_priority: (packet: packet_camera_aim_assist_actor_priority) => void;
		...
		camera_spline: (packet: packet_camera_spline) => void;
		...
		clientbound_attribute_layer_sync: (packet: packet_clientbound_attribute_layer_sync) => void;
		...
		clientbound_data_driven_ui_close_screen: (packet: packet_clientbound_data_driven_ui_close_screen) => void;
		clientbound_data_driven_ui_reload: (packet: packet_clientbound_data_driven_ui_reload) => void;
		clientbound_data_driven_ui_show_screen: (packet: packet_clientbound_data_driven_ui_show_screen) => void;
		...
		clientbound_texture_shift: (packet: packet_clientbound_texture_shift) => void;
		clientbound_update_sound_data: (packet: packet_clientbound_update_sound_data) => void;
		...
		locator_bar: (packet: packet_locator_bar) => void;
		...
		party_changed: (packet: packet_party_changed) => void;
		...
		sync_world_clocks: (packet: packet_sync_world_clocks) => void;
		...
		voxel_shapes: (packet: packet_voxel_shapes) => void;
	}
	...
	export interface ServerboundPacketEventMap {
		...
		party_destination_cookie_response: (packet: packet_party_destination_cookie_response) => void;
		...
		resource_packs_ready_for_validation: (packet: packet_resource_packs_ready_for_validation) => void;
		respawn: (packet: packet_respawn) => void;
		rider_jump: (packet: packet_rider_jump) => void;
		script_custom_event: (packet: packet_script_custom_event) => void;
