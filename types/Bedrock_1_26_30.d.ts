export namespace MCProtocol.Bedrock_1_26_30 {
	...
	export type ItemV4NetIdVariant = {
		type: "item_stack_net_id" | "item_stack_request_id" | "item_stack_legacy_request_id";
		id: number;
	};
	export type ItemV4 = {
		network_id: number;
	...
	/**
	 * As of 1.26.30 the inventory transaction embedded in PlayerAuthInput was NOT updated alongside the one in
	 * InventoryTransaction: it kept the pre-1.26.30 layout. `TransactionActions`/`TransactionLegacy` above
	 * therefore only describe InventoryTransaction; the two types below describe the PlayerAuthInput variant.
	 * Note the actions still carry legacy `Item`s here, not `ItemV4`.
	 */
	export type TransactionActionsAuthInput = {
		source_type: "container" | "global" | "world_interaction" | "creative" | "craft_slot" | "craft";
	...
	/**
	 * Same as `TransactionLegacy`, but `legacy_transactions` is gated on `legacy_request_id` being non-zero
	 * instead of carrying its own presence byte.
	 */
	export type TransactionLegacyAuthInput = {
		legacy_request_id: number;
		legacy_transactions?: {
			container_id: number;
			changed_slots: {
	...
	export type packet_player_auth_input = {
		...
		delta: vec3f;
		transaction?: {
			legacy: TransactionLegacyAuthInput;
			actions: TransactionActionsAuthInput;
